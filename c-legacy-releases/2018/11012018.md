...
| Función | Descripción |
| -----------| ---------- |  
| [Cambios en los grupos de informes virtuales/organización de proyectos](https://marketing.adobe.com/resources/help/es_ES/analytics/analysis-workspace/curate-projects-vrs.html) | (Implementado en septiembre de 2018). Se han realizado cambios en los componentes que los administradores y no administradores pueden ver en los proyectos depurados de Workspace y en los grupos de informes virtuales depurados. Anteriormente, cualquier usuario podía ver los componentes no depurados al hacer clic en el botón **[!UICONTROL Mostrar todos los componentes]**. La experiencia actualizada de depuración permite un control más detallado sobre qué componentes son visibles. |
| Adobe [!DNL Analytics] y el Reglamento general de protección de datos (RGPD) | [!DNL Analytics] admite un nuevo método para pasar el ID de [!DNL Analytics] visitante heredado a una solicitud GDPR (consulte [AAID](https://marketing.adobe.com/resources/help/en_US/analytics/gdpr/gdpr_namespaces.html)). Además, [!DNL Analytics] ahora admite la [eliminación (anonimización) del valor](https://marketing.adobe.com/resources/help/en_US/analytics/gdpr/gdpr_labels.html) de ID de compra como parte de una solicitud de eliminación. |
| [Mejoras del RGPD/ePrivacy para el reenvío en el lado del servidor](https://marketing.adobe.com/resources/help/en_US/analytics/audiences/ssf-gdpr.html) | Prompted by the [EU cookie compliance regulation](http://ec.europa.eu/ipg/basics/legal/cookies/index_en.htm), data controllers ([!DNL Analytics] customers) now have the option to restrict pre-consent data to Adobe [!DNL Analytics], and prevent it from being server-side forwarded to Adobe Audience Manager (AAM). Una nueva variable de contexto de implementación permite marcar las visitas en las que no se ha recibido el consentimiento. Cuando se configura, la variable evita que estas visitas se envíen a AAM hasta que se reciba el consentimiento. |
...
| Mantenimiento de pausas publicitarias | Se mejoró el seguimiento de anuncios en escenarios en los que si un reproductor ha entrado en una pausa publicitaria y no se envían llamadas durante un período de tiempo determinado, la lógica del SDK volverá a la reproducción del contenido de forma predeterminada, incluso si el reproductor sigue en estado de pausa publicitaria. Durante un anuncio previo, esto puede resultar en que se active un inicio de contenido antes de que se complete el desglose de publicidad. En el SDK de Media 2.2, las llamadas de búfer se enviarán entre publicidades dentro de un desglose de publicidad cuando haya un retraso prolongado entre las publicidades. |
| Cambio de nombre del SDK | A partir de la versión 2.2, los SDK de Video Heartbeat Library (VHL) pasan a denominarse SDK de Media. El SDK de Media 2.2 es totalmente compatible con la serie VHL 2.X del SDK. El cambio de nombre no representa un salto funcional, simplemente un cambio en la convención de nombres. |
