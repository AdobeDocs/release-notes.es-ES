...
| -----------| ---------- | ---------- |
| Actualización de recopilación de datos | Añadidos el 27 de junio de 2019 | Todas las cookies de Adobe Analytics con el indicador &quot;cookie de sitio por defecto&quot; tienen ahora este indicador establecido en `SameSite=None`. |
| Cambios en la interfaz de usuario del registro de auditoría | Actualizado el 25 de junio de 2019 | En la API `Logs.GetUsageLog`, se han realizado los siguientes cambios a la IU: <ul><li>Se han cambiado los encabezados de respuesta de la API: a) de `timestamp` a `dateCreated`, b) de `login` a `companyLogin`, c) de `event_num` a `eventType`, d) de `event_type` a `event_type`, e) de `ip_address` a `ipAddress`, f) de `report_suite` a `rsid`, g) de `event_details` a `logText`. </li><li>El formato de marca de hora cambió de Unix a la ISO 8601</li></ul><br>En la API y en la interfaz de usuario del registro de administración, los resultados de los registros de auditoría ahora contienen la ID del grupo de informes en lugar del nombre del grupo de informes. |
...
* Los usuarios pueden utilizar `contains` y `exact match` en su búsqueda. También pueden buscar utilizando los parámetros siguientes:
   ...
   * Tipo de información DITA. Por ejemplo, mapa, tema, concepto, etc.
...
| Protección, solidez y escalabilidad | Se ha añadido una serie de mejoras a [!DNL Campaign Classic]. Guardrail, robustness and scalability improvements are listed in [Adobe Campaign Classic Release Notes](https://docs.campaign.adobe.com/doc/AC/es/RN.html). |
