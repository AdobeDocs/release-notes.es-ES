...
Las mejoras importantes en la Análisis [!UICONTROL de] cohorte le permiten:

* Aplique una inclusión de segmentos y devuelva métricas por separado.
* Mostrar la reproducción en lugar de la retención.
* Mostrar tablas de latencia (tiempo transcurrido antes y después de un evento de inclusión).
* Personalice la dimensión de cohorte (para agrupar visitantes basados en una eVar, no solo en el tiempo).
* Cálculo de cohorte móvil: calcule la retención y la reproducción en base a un período de tiempo anterior, no a la cohorte original.
* Añada en varias métricas en los campos de inclusión y devolución, así como aplique segmentos. (No se admiten las métricas calculadas).
...
Mejoras de velocidad en las visualizaciones de desglose. Por ejemplo, los proyectos con varios desgloses se cargan más rápido.

#### Correcciones en [!UICONTROL Analysis Workspace]

* Se corrigió un problema en el cual las definiciones de segmentos internos creadas al soltar componentes en la zona de colocación del panel no persistían en todas las sesiones. (AN-172110)
* Se ha corregido un problema que impedía guardar las ediciones de etiquetas de visualización. (AN-171119, AN-170287 y AN-169721)
* Se han corregido problemas de rendimiento con [!UICONTROL Analysis Workspace], especialmente con proyectos que contienen visualizaciones de varias líneas. (AN-169916, AN-169691, AN-167120 y AN-168082)
...
* Se corrigió un problema en el cual los datos de desglose no se actualizaban cuando se contraía la tabla superior y se cambiaba el intervalo de fechas. (AN-168695)
...
* Se corrigió un problema en el cual los informes a los que se accedía mediante vínculos cortos y que tenían ampersands en el filtro de búsqueda perdían cierta información del filtro. (AN-167151)
* Se ha corregido un problema que provocaba visualizaciones en blanco en los paneles de informes descargables. (AN-171273)
* Se ha corregido un problema que provocaba un envío fallido de los informes HTML programados que contenían comparaciones de fechas. (AN-168990)
* Se ha corregido un problema que impedía editar segmentos. (AN-172307, AN-169785)
...
* Se ha corregido un problema que impedía a los usuarios guardar los cambios en los grupos de informes virtuales vinculados a segmentos eliminados. (AN-168906)
* Se ha corregido un problema que provocaba que las métricas de participación dejaran de funcionar con variables de lista en la Ad hoc analysis. (AN-166027)
...
* Hemos mejorado la experiencia de búsqueda en Audience Marketplace para proporcionar una experiencia más rápida, sin interrupciones. (AAM-43421)
* Hemos realizado una actualización en la interfaz de usuario del Audience Marketplace para permitirle introducir valores decimales con coma para impresiones de Payables. (AAM-42994)
* Hemos corregido un error en Audience Marketplace, en el que los clientes que desean informar sobre volúmenes de impresión en diciembre encontrarían las impresiones de noviembre que aún se muestran en los campos de sistema de informes. (AAM-43814)
...
* Para la publicación de contenido fragmentado.
* En temas, informes, fichas de traducción del panel del mapa.
* UX de recursos.
* Correcciones de rendimiento para revisiones de mapas grandes.
...
* Compatibilidad con mapa de imagen.
* Ordenación alfabética para una mejor experiencia.

#### Mejoras en la conversión de Word a DITA

* Administre estilos en línea como negrita, cursiva, listas, etc. en Word para convertir DITA.
* Se ha mejorado el procesamiento de tablas en la conversión de Word a DITA.
* El proceso de conversión ahora crea los temas con los nombres de archivo formados con el título de archivo con _(FILE_NUMBER) anexado al final para los archivos DITA.
* No se insertan atributos xtrc adicionales dentro de los elementos DITA durante el proceso de conversión.

#### Compatibilidad con API

* API de Java para crear un atributo de creación de perfiles para un perfil de carpetas.
* API de etiquetado: proporcione API para aplicar una etiqueta determinada a todas las versiones especificadas en una línea base.
* API de línea de base: cree una línea de base para un mapa con versiones en una fecha y hora determinadas.
...
* Compatibilidad con variables y campos de metadatos en los nombres de sitios de PDF y AEM.
* Ahora puede superponer el cuadro de diálogo Buscar en el Editor de asignaciones con un componente personalizado.
* Permite a los usuarios asociar varias carpetas con un perfil de carpetas.
* Compatibilidad con AEM 6.4 SP2.

#### Errores informados por los clientes que han sido corregidos

* La columna Cierre de compra no se mostraba correctamente en la Vista de Lista de AEM Assets.
* Página de recopilación de mapas para un gran número de colecciones de mapas.
* Los grupos DITA predeterminados se eliminaron al desinstalar el paquete.
* El usuario debería tener la opción de registrarse cuando cierre un archivo extraído.
* El botón Descartar sin sincronizar no está visible en el panel de traducción.
* Se genera un TDC incorrecto si el fragmentación está activado.
* El Editor XML de AEM desordena las columnas cargadas.
...
* El componente TDC utilizado por la plantilla de sitio predeterminada procesa vínculos incorrectos cuando está en vigor una asignación de resolución de recursos JCR.
...
Si desea utilizar esta estrategia para un portafolio, espere hasta que el portafolio se haya optimizado durante al menos dos semanas y, a continuación, supervise activamente el gasto y el rendimiento hasta que se valide la característica.
