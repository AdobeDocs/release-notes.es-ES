...
* Se ha corregido un problema en el cual el nombre/apellido del usuario (migrado) de Analytics era diferente al nombre vinculado al Adobe ID del usuario en Experience Cloud. Para actualizar el nombre de Analytics con el de Experience Cloud, vaya a **[! UICONTROL Admin** &gt; **!UICONTROL User Management (Heredado)**&gt;**!UICONTROL Editar usuarios** y haga clic en el ID de usuario. A continuación, haga clic en **!UICONTROL Actualizar desde Experience Cloud **. (AN-172825)
...
* La [página de resumen del segmento](https://experiencecloud.adobe.com/resources/help/en_US/aam/c_segment_summary.html) ahora contiene un hipervínculo al origen de datos del segmento para facilitar la navegación. (AAM-40015)
...
* Se ha corregido un error donde la casilla **[!UICONTROL Seleccionar todo]** faltaba en Modelos algorítmicos, en [la](https://experiencecloud.adobe.com/resources/help/en_US/aam/trait-exclusion-algo-models.html) página Seleccionar características a Excluir. (AAM-42462)
...
Leer [el monitoreo de ID y la lista negra](https://marketing.adobe.com/resources/help/en_US/aam/id-monitoring-blacklisting.html) para comprender cómo el DCS supervisa los ID que recibe y la lista negra de los que se envían a una tasa inusualmente alta en un corto período de tiempo.
...
Para obtener más información, consulte [Informe de notificaciones Push](https://helpx.adobe.com/campaign/standard/reporting/using/push-notification-report.html).
...
Para más información, consulte [Uso del kit de desarrollo de software (SDK) de Adobe Experience Platform](https://helpx.adobe.com/campaign/standard/administration/using/configuring-a-mobile-application.html#main-pars_header_49).

### Mensajería móvil en la aplicación

Este lanzamiento contiene la versión GA del canal In-App en Campaign. Desde el punto de vista funcional, las adiciones más importantes a la versión Beta son los informes dinámicos para el canal In-App y el protocolo de enlace seguro entre el SDK de dispositivos móviles y el MCIAS (Servicio de mensajería In-App de Marketing Cloud que sirve para las reglas del SDK). El protocolo de enlace seguro garantiza que los datos PII de los usuarios no acaben en manos malintencionadas y permite mantener la privacidad de los usuarios en un dispositivo compartido eliminando la caché de mensajes cada vez que el usuario cierra la sesión.

Para más información, consulte [Mensajería Móvil In-App](https://helpx.adobe.com/campaign/standard/channels/using/about-in-app-messaging.html) y el [tutorial dedicado a In-App](https://helpx.adobe.com/campaign/kt/acs/using/acs-in-app-message-tutorial.html).
...
* Al utilizar la actividad **Cargar archivo**, ahora se puede añadir una marca de hora al nombre del archivo que contiene los registros rechazados. Para obtener más información, consulte la [documentación detallada](https://helpx.adobe.com/campaign/standard/automating/using/load-file.html#configuration).
...
Consulte las [notas de la versión](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/release-notes/release-notes-current.html) correspondientes.

**AEM 6.5**

[Depuración del nuevo Adobe Experience Manager: Mejoras en la nube](https://theblog.adobe.com/adobe-experience-manager-2019-cloud-content-personalization-CXM/), el contenido y personalización de la administración avanzada de la experiencia del cliente (CXM) para comerciantes y TI.
...
Para obtener más información, consulte [Novedades en AEM Assets Brand Portal](https://helpx.adobe.com/experience-manager/brand-portal/using/whats-new.html) y las [Notas de la versión](https://helpx.adobe.com/experience-manager/brand-portal/release-notes/brand-portal-release-notes.html).
...
Consulte [notas de la versión de Livefyre](https://docs.adobe.com/content/help/en/livefyre/using/release-notes/c-rn.html) para más información.

## Advertising Cloud {#adcloud}

| Función | Descripción |
| -----------| ---------- |  
| Portafolios y campañas de búsqueda | En Detalles de unidad de oferta, las fichas Historial y Previsión de todos los portafolios ahora incluyen fichas independientes para Ordenadores y Tablets. Anteriormente, las fichas independientes solo estaban disponibles para portafolios utilizando la versión 7 de la capacidad de optimización. <br/> Para los portafolios con la versión 7, todos los datos de las fichas Ordenadores y Tabletas son independientes.<br/>Sin embargo, para portafolios con la versión 6 de la capacidad de optimización, tanto las fichas Equipos como Tabletas muestran los totales combinados para equipos y tablets, que se mostraban anteriormente para Ordenadores + Tablets.<br/>**Nota:** Cuando el método de optimización cambia de v 6 a v 7 y viceversa, los datos del historial se restauran el mismo día. |
| Buscar campañas | Ahora puede habilitar y deshabilitar varias cuentas de motores de búsqueda a la vez.<br/>(Cuentas de Bing Ads) Las audiencias personalizadas de Bing Ads existentes ahora están incluidas en la vista Audiencias &gt; Biblioteca y están disponibles como objetivos para grupos de anuncios de Bing Ads. Opcionalmente, puede definir un modificador de oferta para cada destino de audiencia. |
