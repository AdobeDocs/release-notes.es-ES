...
* Se ha añadido ` idSyncAttachIframeOnWindowLoad` como indicador booleano opcional que puede establecer en la función ` Visitor.getInstance`. Cuando ` idSyncAttachIframeOnWindowLoad= true`, el servicio de ID carga la sincronización iFrame de ID en la ventana de carga. El servicio de ID carga el iFrame lo más rápido posible de forma predeterminada. Este indicador *sustituye* el ` idSyncAttachIframeASAP` en desuso. Consulte [Variables utilizadas por la función getInstance del servicio de ID](https://docs.adobe.com/content/help/en/id-service/using/home.html/mcvid-variables.html). (MCID-122)
* Funcionalidad agregada para admitir un seguimiento de los ID de [!DNL  Marketing Cloud] entre dominios, aplicaciones nativas e híbridas, y transiciones web. Consulte [Función de ayuda para asignar un ID de visitante](https://docs.adobe.com/content/help/en/id-service/using/home.html/mcvid-appendvisitorid.html). (MCID-42)
* Funciones agregadas para el código VisitorAPI.js que determinan si el servicio de ID ha generado el ID de [!DNL  Marketing Cloud] del lado del cliente o lado del servidor, o si se agotó el tiempo de espera de las llamadas de ID. Consulte las [funciones de seguimiento de tiempo de espera](https://docs.adobe.com/content/help/en/id-service/using/home.html/mcvid-timeout-functions.html) y [la generación de seguimiento de la ID de visitante del lado del cliente](https://docs.adobe.com/content/help/en/id-service/using/home.html/mcvid-client-side-id.html). (MCID-116)

**Problemas conocidos**

Los clientes que utilizan código DIL de [!DNL  Audience Manager] y código visitorAPI.js en la misma página, deben establecer la variable DIL ` secureDataCollection= false`. Consulte [secureDataCollection](https://marketing.adobe.com/resources/help/en_US/aam/dil-secure-data-collection.html).
...
<table id="table_1207853963C64C5E95E85E9B34B6FCFE"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> Aumento del tiempo de espera de 30 a 60 segundos </td> 
...
<table id="table_91D1FD20C4C1411292252364328677AF"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>Comunicación ampliada de las fuentes de datos a las regiones de AWS (AN-118446) </p> </td> 
   <td colname="col2"> <p>Las fuentes de datos ahora pueden comunicarse con las 11 regiones estándar de AWS siguientes (mediante el algoritmo de firma adecuado cuando sea necesario): </p> 
    ...
    </ul> <p>Actualmente no apoyamos la región de AWS de Pekín y China (cn-norte-1). </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Data Warehouse: ayuda para clasificar elementos de línea (AN-125562) </p> </td> 
   <td colname="col2"> <p>Ahora los informes del Data Warehouse son compatibles con la clasificación de elementos de línea llevada a cabo por la métrica o métricas suministradas. Cuando la ordenación está en uso, si también decide limitar el número de filas en la tabla (página de configuración avanzada de DW), el informe devolverá los valores "Top N" con respecto a la métrica por la que está ordenando. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Data Warehouse: cambio de la denominación lógica de columna </p> </td> 
   <td colname="col2"> <p> <b>En la versión de mantenimiento de julio</b> se cambió la denominación lógica de columna del <span class="wintitle">Data Warehouse</span>. Anteriormente, si el nombre de evar1 era "ABC", el nombre de la columna sería "ABC (evar1)". </p> <p>En los casos en que la variable ya contenía el identificador, esto se volvió redundante: </p> 
...
<!-- <draft-comment> 
    ...
    </ul> <p> <p>Nota: También puede consultar las notas de la versión 1.6.3 de JavaScript del <a href="https://marketing.adobe.com/resources/help/es_ES/sc/appmeasurement/release/c_release_notes_mjs.html" format="html" scope="external">4 de agosto</a>, las cuales incluyen requisitos actualizados para el servicio de Experience Cloud ID. </p> </p> 
...
<!-- <draft-comment> 
    ...
    </draft-comment> --> <p> <b>Ver también:</b> <a href="../../c-legacy-releases/2016/08182016.md#mcvid" format="dita" scope="local"> Servicio de Experience Cloud ID </a> (antes <span class="term">Servicio de ID de visitante</span>) </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_A6749BA62D5B40479B949EA1C64E4B7F"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>Audiencia  Laboratorio </p> </td> 
...
DIL es el código de recopilación de datos de API para [!DNL  Audience Manager]. La versión de julio actualiza el DIL a la versión 6.5. Actualice a la versión más reciente para aprovechar estas funciones y correcciones.
...
* Se ha añadido la compatibilidad de DIL para el Intercambio de recursos de origen cruzado (CORS). Consulte [useCorsOnly](https://marketing.adobe.com/resources/help/en_US/aam/dil-use-cors-only.html). (AAM-25319, AAM-20139).
* El DIL iFrame es compatible con los atributos de ` iframe.sandbox='allow-scripts allow-same-origin';`. Al permitir solamente estos 2 tokens, ayuda a mejorar la seguridad. 	Para obtener más información consulte la sección Atributos de la [documentación de iFrame](https://developer.mozilla.org/es-ES/docs/Web/HTML/Element/iframe). (AAM-23335)
...
* Frecuencia aumentada y vida útil de recopilación de datos de métricas de Facebook. El nuevo intervalo de tiempo de recopilación de datos es el siguiente:

   * Todas las métricas de Facebook se recopilan cada hora durante las primeras 72 horas posteriores a la publicación.
   * Continúe la recopilación de datos cada 8 horas durante las siguientes 72 horas.
   * A continuación, continúe la recopilación de datos diariamente durante los 60 días siguientes.

* Se añadió una columna de Geotarget para Twitter en la exportación de informes de Análisis de anuncio.
* Cinco nuevas métricas de reacciones de Facebook ya están disponibles en los informes Análisis de publicaciones y Análisis de propiedades. Incluyen: Amor, Wow, Triste, Ja, y Enfadado. En la próxima versión, las métricas de reacciones de Facebook también estarán disponibles en los informes de Adobe Analytics.
...
* Se corrigió un problema en el cual los informes de Social Buzz generados en diferentes días soltaban ciertas publicaciones.
* Se corrigió un problema en el cual el cambio de la configuración de fuente dentro de Moderación unificada no se guardaba después de cerrar la sesión y volver a iniciarla.
* Se corrigió un problema en el cual las direcciones URL de los tweets citados ocupaban más de los 24 caracteres esperados.
...
<table id="table_04819C0464594ECFB211C253C34E69CA"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> Adobe ha lanzado correcciones rápidas de seguridad para Adobe Experience Manager. Estas correcciones rápidas resuelven dos problemas de validación que podrían utilizarse en ataques de secuencias de comandos entre sitios (CVE-2016-4168 y CVE-2016-4170): </p> 
...
La siguiente tabla contiene una selección de vínculos a temas, eventos y seminarios Web sugeridos que pueden ayudarle a aprovechar al máximo la implementación y el uso de AEM.
...
<table id="table_8EE2C5FD9964403D91FFBCDF5FECEACE"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Únase a compañeros de su zona o a una reunión de un grupo de usuarios si se encuentra de viaje. En el calendario de la Comunidad puede obtenerse una lista de cada evento. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Únase a sus compañeros en AEM Eventos para desarrolladores </p> </td> 
   <td colname="col2"> <p>Los siguientes son dos próximos eventos de @AEM_Dev: </p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>El 23 de agosto de 2016, únase a Lokesh BS y Scott Macdonald para una discusión sobre cómo usar AEM comercio electrónico. </p> <p>La sesión abarca el marco de comercio electrónico, incluida la forma de utilizar las API de comercio electrónico. Al unirse a esta sesión, podrá comprender mejor cómo utilizar el marco de comercio electrónico de AEM y las API asociadas. </p> <p> <a href="https://communities.adobe.com/content/usergenerated/content/cush/en/communities/aem_technologistsdevelopersarchitects/events/_jcr_content/par/calendar/ask_the_aem_communit_3.form.html/content/cush/en/communities/aem_technologistsdevelopersarchitects/events/upcoming-event-detail" format="https" scope="external"> Regístrese aquí para unirse a la sesión </a>. </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>El 17 de agosto de 2016, asista al seminario web "Cómo configurar y configurar los medios dinámicos". </p> <p>Entre los temas tratados en el seminario web se incluyen los siguientes: </p> 
    <ul id="ul_6C2DD857C8804D19A8F0D7FE51C6745C"> 
     <li id="li_864C04A369E342ED9C4F0DED93D9E744"> Casos de uso de Dynamic Media. </li> 
     <li id="li_22E96702DD0E41CBAA101331B0FAABF5"> Arquitectura técnica. </li> 
     <li id="li_68EB914F76D343148E14971E5930E9E1"> Cómo configurar Dynamic Media en AEM 6.2. </li> 
     <li id="li_5F9B644D992840F395477A340E078544">Configuración y prácticas recomendadas para imágenes de Dynamic Media. </li> 
     <li id="li_CF200832AB4A4B53B98434EA8DEAFE5F">Configuración y prácticas recomendadas para vídeo de Dynamic Media. </li> 
